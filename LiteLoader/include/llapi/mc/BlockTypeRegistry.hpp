...
public:
    ...
    MCAPI static class WeakPtr<class BlockLegacy> lookupByName(std::string const &);
    MCAPI static void unregisterBlock(std::string const &);

//private:

private:
    MCAPI static class std::unordered_map<std::string, class SharedPtr<class BlockLegacy>, struct std::hash<std::string>, struct std::equal_to<std::string>, class std::allocator<struct std::pair<std::string const, class SharedPtr<class BlockLegacy>>>> mBlockLookupMap;
